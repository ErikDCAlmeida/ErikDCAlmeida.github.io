...
obj(".menuScreenLeft").addEventListener("click", (e) => {
  if (e.target.id === "wpp") {
    window.open(
      "https://wa.me/5575988681299/?text=Olá,%20Erik.%20Gostaria%20de%20entrar%20em%20contato%20com%20você!"
    );
  } else if (e.target.id === "lkd") {
...
projetosJSON.map((item, index) => {
  ...
  projeto.querySelector(
    ".projeto"
  ).style.backgroundImage = `url(${item.image})`;
  projeto.querySelector(
    ".bgProjeto"
  ).innerHTML = `<span>${item.nome}</span><span>${item.ferramenta}</span>`;
  projeto.querySelector(
    ".projeto"
  ).style.boxShadow = `0px 0px 5px 2px ${item.color}`;
